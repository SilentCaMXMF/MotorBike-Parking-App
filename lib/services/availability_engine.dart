...
class AvailabilityEngine {
  ...
  static int calculateCurrentOccupancy(List<UserReport> reports) {
    ...
    final validReports = reports
        .where((r) => r.reportedCount >= 0 && r.userId.isNotEmpty)
        .toList();
    ...
    for (final UserReport report in validReports) {
      userReportCounts[report.userId] =
          (userReportCounts[report.userId] ?? 0) + 1;
    }
    ...
    final weightedSum = validReports.map((report) {
      ...
      final recencyWeight = age <= 30
          ? 1.0
          : age <= 60
              ? 0.5
              : 0.2;
      final userReputation =
          (userReportCounts[report.userId] ?? 1).clamp(1, 5).toDouble() /
              5.0; // Max 5 reports for full reputation
    ...
    final totalWeight = validReports.map((report) {
      ...
      final recencyWeight = age <= 30
          ...
          : age <= 60
              ...
              : 0.2;
      final userReputation =
          (userReportCounts[report.userId] ?? 1).clamp(1, 5).toDouble() / 5.0;
  ...
  static double calculateConfidenceScore(
      List<UserReport> reports, int capacity) {
    ...
    final recentReports = validReports
        .where((r) => now.difference(r.timestamp).inMinutes <= 30)
        .toList();
    ...
    if (validReports.length > 1) {
      ...
      consistencyScore =
          capacity > 0 ? 1.0 - (stdDev / capacity).clamp(0.0, 1.0) : 0.0;
    } else {
      consistencyScore = 0.5; // Neutral for single report
    }

    // Volume score (more reports = higher confidence)
    volumeScore = (validReports.length / 5.0)
        .clamp(0.0, 1.0); // Max 5 reports for full score

    // Weighted average
    return (recencyScore * 0.5 + consistencyScore * 0.3 + volumeScore * 0.2)
        .clamp(0.0, 1.0);
  }

  /// Updates a parking zone with new occupancy calculations from user reports.
  /// Returns the original zone if capacity is invalid.
  static ParkingZone updateZoneWithReports(
      ParkingZone zone, List<UserReport> reports) {
...
extension on double {
  double sqrt() => this < 0
      ...
      : this == 0
          ? 0
          : (this * 0.5 + this / (this * 0.5)) * 0.5; // Approximation
}
